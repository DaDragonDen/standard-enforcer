...
import { Client, Message, AnyGuildTextChannel, AnyGuildChannelWithoutThreads } from "oceanic.js";

export default async (collection: Collection, bot: Client, newMessage: Message<AnyGuildTextChannel>) => {
  ...
  if (!archiveThread && archiveThreadId) {
    ...
    const channels: AnyGuildChannelWithoutThreads[] = guild.channels.filter(channel => channel.type === 0);
    for (let i = 0; channels.length > i; i++) {

      try {

        const channel = await bot.rest.channels.get<AnyGuildChannelWithoutThreads>(channels[i].id);
        if (channel.type === 0) {
